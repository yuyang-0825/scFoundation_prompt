...
def select_module(config, sub_config, module_name,prompt_settings):
...
def select_model(config,prompt_settings):
    print(prompt_settings)
    if config["model"] == "mae_autobin":
        encoder_config =config['encoder']
        decoder_config = config['decoder']
        encoder = select_module(config, encoder_config, config['encoder']['module_type'],prompt_settings)
        decoder = select_module(config, decoder_config, config['decoder']['module_type'],prompt_settings)
        model = MaeAutobin(
