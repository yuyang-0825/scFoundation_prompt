...
def main():
    ...
    pretrainmodel,pretrainconfig = load_model_frommmf(ckpt_path,prompt_settings,key)

    pre_freeze_param_count = sum(
        dict((p.data_ptr(), p.numel()) for p in pretrainmodel.parameters() if p.requires_grad).values())
    keywords = ('lora', 'adapter', 'Adapter')
    for name, para in pretrainmodel.named_parameters():
        para.requires_grad = False
    params_to_update = filter(lambda p: any(keyword in p[0] for keyword in keywords), pretrainmodel.named_parameters())
    for _, param in params_to_update:
        param.requires_grad = True

    print("-" * 89)
    learnable_params = {k: v for k, v in pretrainmodel.named_parameters() if v.requires_grad == True}
