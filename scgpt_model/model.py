...
class PeftTransformerEncoderLayer(nn.Module):
    ...
    def __init__(self, d_model: int, nhead: int, dim_feedforward: int = 2048, dropout: float = 0.1,
        ...
        OFT_FLAG = peft_config.pop("OFT_FLAG", False)
        ...
        if OFT_FLAG:
            # print('use OFT in model.py',flush=True)
            # try:
            #     from .MultiheadAttentionOFT import MultiheadAttentionOFTImpl
            #     if adapter_layer_conf[index]:
            #         self.self_attn = MultiheadAttentionOFTImpl(
            #             d_model,
            #             nhead,
            #             dropout=dropout,
            #             batch_first=batch_first,
